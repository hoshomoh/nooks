 ...
 *
 * A build succeeds with broken navigation, which is a build that cannot be trusted.
 * Pointing a nav entry at a folder rather than a page is enough to do it.
 *
 * Checked against the output, not the sources: what matters is whether the page a
 * reader arrives at exists, not whether an MDX file with a similar name does.
 */
...
// routeOf is the path a reader types to reach a built file. `index.html` is the
// directory it sits in, not a page called "index"; at the root that is `/`.
