...
func migrate(ctx context.Context, db *sql.DB, d dialect) error {
	...
	pending, err := pendingMigrations(d, applied)
	if err != nil {
		return err
	}
	for _, name := range pending {
		if err := applyMigration(ctx, db, d, name); err != nil {
			return err
		}
	}
	return nil
...
func pendingMigrations(d dialect, applied map[string]bool) ([]string, error) {
	dir := path.Join("migration", d.name)
	entries, err := fs.ReadDir(migrations, dir)
	if err != nil {
		return nil, fmt.Errorf("read migrations for %s: %w", d.name, err)
	}
...
func applyMigration(ctx context.Context, db *sql.DB, d dialect, name string) error {
	statements, err := migrations.ReadFile(path.Join("migration", d.name, name))
	if err != nil {
	...
	record := fmt.Sprintf(`INSERT INTO schema_migration (name) VALUES (%s)`, d.placeholder(1))
